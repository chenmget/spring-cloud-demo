...
import com.iwhalecloud.retail.order2b.service.workflow.PurApplyGetHandlerUserIdService;
...
@Slf4j
@Service
public class PurApplyGetHandlerUserIdServiceImpl implements PurApplyGetHandlerUserIdService {
    ...
    public ResultVO run(InvokeRouteServiceRequest params) {
        log.info("PurApplyGetHandlerUserIdServiceImpl.run params={}", JSON.toJSONString(params));
        if (params == null || StringUtils.isEmpty(params.getParamsValue())) {
            return ResultVO.error(ResultCodeEnum.LACK_OF_PARAM);
        }

        String merchantId = params.getParamsValue();
