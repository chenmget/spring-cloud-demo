...
import com.iwhalecloud.retail.web.controller.b2b.member.requst.LoginReq;
import com.iwhalecloud.retail.web.controller.b2b.member.requst.VerificationCodeReq;
import com.iwhalecloud.retail.web.controller.b2b.member.response.LoginResp;
...
public class MemberController extends BaseController {
	...
	public ResultVO<LoginResp> memberLoginOrRegister(HttpServletRequest request,@RequestBody LoginReq loginReq) {
		...
		if (StringUtils.isEmpty(loginReq.getMobile())) {
			return ResultVO.error(ResultCodeEnum.ERROR.getCode(), "用户名或手机号不能为空");
		}
		if (StringUtils.isEmpty(loginReq.getPwd())) {
			return ResultVO.error(ResultCodeEnum.ERROR.getCode(), "密码或验证码不能为空");
		}
		if (StringUtils.isEmpty(loginReq.getLoginType())) {
			return ResultVO.error(ResultCodeEnum.ERROR.getCode(), "登陆类型不能为空");
		}
...
//            memberLoginResp = memberService.login(req);
			return ResultVO.error(ResultCodeEnum.ERROR.getCode(), "登陆类型错误,暂时只支持验证码登录");
		} else if (WebConst.LoginTypeEnum.VERIFICATION_CODE.getCode().equals(loginReq.getLoginType())){
			req.setUserName(loginReq.getMobile());
			memberLoginResp = memberService.login(req);
		} else {
			return ResultVO.error(ResultCodeEnum.ERROR.getCode(), "登陆类型错误");
		}
	...
	private boolean registerByMobile(String mobile) {
		...
		// 注册失败
		if(!rigistResult.getResultCode().equals(ResultCodeEnum.SUCCESS.getCode())){
			return false;
		}
		return true;
	}
