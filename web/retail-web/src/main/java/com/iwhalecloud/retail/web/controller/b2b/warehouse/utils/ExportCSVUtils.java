...
public class ExportCSVUtils {
	 ...
	 * @param map  导出数据对应字段
	 */
	public static void exportItms(OutputStream output, List list,List<ExcelTitleName> map) {
		try {
			StringBuffer buf = new StringBuffer();
	...
	private static String transferValue(String filedName, String value) {
		...
		if (CREATE_DATE.equals(filedName)) {
			try {
				Date date = new Date(Long.valueOf(value));
				String StringDate = format.format(date);
				finalValue =  StringDate;
		...
		if (STATUS_CD.equals(filedName)) {
			if (MarketingResConst.ResultEnum.FAIL.getCode().equals(value)) {
				finalValue = "失败";
			}else {
				finalValue = "成功";
			}
