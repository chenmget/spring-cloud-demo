...
import com.iwhalecloud.retail.partner.service.workflow.FactoryMerchantSaveNotPassService;
...
@Component
@Service
public class FactoryMerchantSaveNotPassServiceImpl implements FactoryMerchantSaveNotPassService {
    @Autowired
    private MerchantService merchantService;

    @Override
    public ResultVO run(InvokeRouteServiceRequest params) {
        log.info("FactoryMerchantPermissionApplyNotPassServiceImpl.run params={}", JSON.toJSONString(params));
        if (params == null) {
        ...
        //商家状态变为无效
        merchantUpdateReq.setStatus(PartnerConst.MerchantStatusEnum.INVALID.getType());
